...
> Use these when the project does not have an MCP server for Linear/Jira/GitHub. The agent or user can run these in the shell to fetch ticket details, or use `agent/fetch-ticket.sh` if it was created by install.

## Token sources

- **Environment:** `LINEAR_API_TOKEN`, `JIRA_API_TOKEN` + `JIRA_URL`, `GITHUB_TOKEN` + `GITHUB_REPO`
- **Rails:** If not in env, you can use `rails runner "puts ENV['LINEAR_API_TOKEN']"` (or from credentials) for token lookup in a Rails app.
...
linear_issue() {
  ...
  # Optional: in a Rails project, fallback to Rails env
  if [ -z "$TOKEN" ] && command -v rails &>/dev/null; then
    TOKEN=$(rails runner "puts ENV['LINEAR_API_TOKEN']" 2>/dev/null)
  fi
...
**Usage:** `linear_issue "<uuid>"` — use the issue UUID from Linear (e.g. from URL or API). Output is raw JSON; pipe to `jq -r '...'` for markdown if needed.

**Markdown-friendly output (optional):**
...
2. **Otherwise**  
   Copy the functions above into a script or source a file that defines them, set the required env vars (or use Rails runner for `LINEAR_API_TOKEN` in a Rails app), then run e.g.:
