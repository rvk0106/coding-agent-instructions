...
- **Conventions:** `workflow/implementation.md` — file locations, Rails conventions, API response shape, danger zones
...
| Matches API/response shape | | Per implementation.md |
| Tests added/updated as required | | Specs for new/changed behavior |
| Danger zones avoided or explicitly called out | | Auth, DB, money, secrets, tenant, jobs |
| Verification commands pass | | RSpec, RuboCop (and any phase-specific) |
...
- Matches API/response shape: [Pass/Fail] — [note]
