...
- **Conventions:** `workflow/implementation.md` -- file locations, Rails conventions, view conventions, danger zones
- **Verification:** `workflow/testing.md` -- test and lint commands that must pass

### 4. Run verification
- Execute the phase's (or ticket's) verification commands (e.g. `bundle exec rspec ...`, `bundle exec rubocop`)
...
| Views follow conventions | | Partials, locals, flash, Turbo status codes |
| Tests added/updated as required | | Specs for new/changed behavior |
| Danger zones avoided or explicitly called out | | Auth, DB, money, secrets, tenant, jobs, XSS |
| Verification commands pass | | RSpec, RuboCop, system specs (and any phase-specific) |
...
- Views follow conventions: [Pass/Fail] -- [note]
