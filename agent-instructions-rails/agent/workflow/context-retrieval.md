...
- **Metadata (per chunk):** `source` (e.g. `architecture/api-design.md`), `section`, optional `task_types` (e.g. `["new_endpoint", "bug_fix"]`) for hybrid filter + vector search.
...
- One entry per logical section (e.g. “API response shapes”, “tenant scoping rules”, “Users feature behavior”).
