...
- **Metadata (per chunk):** `source` (e.g. `architecture/views.md`), `section`, optional `task_types` (e.g. `["new_page", "new_form"]`) for hybrid filter + vector search.
...
1. **Index file:** Maintain a single compact file (e.g. `agent/context-index.yaml` or `agent/context-index.json`) that lists every knowledge unit with:
   - **id** (e.g. `architecture.views.turbo-frames`)
...
- One entry per logical section (e.g. "Turbo Frame patterns", "tenant scoping rules", "Users feature behavior").
- Short summary so the agent (or a small classifier) can decide relevance.
- Pointer to file and section (or line range) so the exact content can be loaded on demand.
