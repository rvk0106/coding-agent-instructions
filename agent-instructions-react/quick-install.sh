#!/usr/bin/env bash
# Quick install script for agent-instructions-react
# Usage: curl -fsSL https://raw.githubusercontent.com/rvk0106/coding-agent-instructions/main/agent-instructions-react/quick-install.sh | bash
# Or with custom directory: curl ... | bash -s /path/to/react/project

set -euo pipefail

REPO_URL="https://github.com/rvk0106/coding-agent-instructions"
FRAMEWORK_DIR="agent-instructions-react"
BRANCH="main"
TEMP_DIR=$(mktemp -d)
TARGET_DIR="${1:-.}"
...
echo "🚀 Installing agent-instructions-react..."
...
# Check if it looks like a React/Node.js project
if [[ ! -f "$TARGET_DIR/package.json" ]]; then
  echo "⚠️  Warning: This doesn't look like a React project (no package.json)."
  if [[ -t 0 ]]; then
...
echo "2. Plan a ticket: plan frontend for TICKET-ID"
