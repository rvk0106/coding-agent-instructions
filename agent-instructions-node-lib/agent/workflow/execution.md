...
## Node.js Library Phase Checks
- If touching exports → confirm TypeScript types updated + backward compatible
- If adding functions → confirm test coverage for all public functions
- If changing errors → confirm error class hierarchy consistency + error codes
- If modifying configuration → confirm defaults and validation
- If adding dependencies → confirm package.json updated + bundle size checked
