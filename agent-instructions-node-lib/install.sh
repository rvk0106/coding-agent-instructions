...
if [[ -z "$TARGET_DIR" ]]; then
  echo "Usage: ./install.sh /path/to/node-library"
  exit 1
fi
...
COPILOT_BLOCK="Follow agent/master-instructions.md as the single source of truth for Node.js library development. Plan first, execute one phase at a time, verify, then stop for review."
CURSOR_BLOCK="Follow agent/master-instructions.md. Planning and execution are separate; no phase auto-continue."
CLAUDE_BLOCK="Use agent/master-instructions.md as your system prompt or initial context for Node.js library development."
