#!/usr/bin/env bash
# Quick install script for agent-instructions-springboot
# Usage: curl -fsSL https://raw.githubusercontent.com/rvk0106/coding-agent-instructions/main/agent-instructions-springboot/quick-install.sh | bash
# Or with custom directory: curl ... | bash -s /path/to/springboot/project

set -euo pipefail

REPO_URL="https://github.com/rvk0106/coding-agent-instructions"
FRAMEWORK_DIR="agent-instructions-springboot"
BRANCH="main"
TEMP_DIR=$(mktemp -d)
TARGET_DIR="${1:-.}"
...
echo "🚀 Installing agent-instructions-springboot..."
...
if [[ ! -f "$TARGET_DIR/pom.xml" ]] && [[ ! -f "$TARGET_DIR/build.gradle" ]] && [[ ! -f "$TARGET_DIR/build.gradle.kts" ]]; then
  echo "⚠️  Warning: This doesn't look like a Spring Boot project."
  if [[ -t 0 ]]; then
