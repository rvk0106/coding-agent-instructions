...
- **Conventions:** `workflow/implementation.md` -- file locations, Spring Boot conventions, API response shape, danger zones
- **Verification:** `workflow/testing.md` -- test and lint commands that must pass

### 4. Run verification
- Execute the phase's (or ticket's) verification commands (e.g. `./mvnw test -Dtest=...`, `./mvnw checkstyle:check`)
...
| Controllers thin, DTOs used (not entities) | | Per implementation.md |
| @Valid on request DTOs | | Bean Validation annotations present |
| @Transactional on multi-step service methods | | Where applicable |
| Tests added/updated as required | | Unit + integration tests for new/changed behavior |
| Danger zones avoided or explicitly called out | | Auth, DB, money, secrets, tenant, async |
| Verification commands pass | | JUnit, Checkstyle (and any phase-specific) |
...
- Controllers thin, DTOs used: [Pass/Fail] -- [note]
- @Valid on request DTOs: [Pass/Fail] -- [note]
- @Transactional where needed: [Pass/Fail] -- [note]
