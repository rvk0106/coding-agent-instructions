...
- **Location:** Index can live under `agent/vector_store/` or in a separate service; document the query interface in `agent-config.md` or README.
...
### When to regenerate
- After **onboarding** and after **maintenance** updates, regenerate the index from the current knowledge files.
