...
|-------------|--------|:-------------------:|
| Java/Spring Boot version, env vars | `infrastructure/environment.md` | Yes |
| Added/removed dependency or service | `infrastructure/dependencies.md` | Yes |
| CI/CD, linter, test config | `infrastructure/tooling.md` | Yes |
| Deploy process, hosting | `infrastructure/deployment.md` | Yes |
| Security config, auth changes | `infrastructure/security.md` | Yes |
| DB schema, tables, indexes, migrations | `architecture/database.md` | Yes |
| API endpoints, response shapes | `architecture/api-design.md` | Yes |
| Design patterns, conventions | `architecture/patterns.md` | Yes |
| System design, components | `architecture/system-design.md` | Yes |
| Error handling, exception hierarchy | `architecture/error-handling.md` | Yes |
| Request lifecycle, filter chain | `architecture/data-flow.md` | Yes |
| Domain terms, roles, statuses | `architecture/glossary.md` | Yes |
| New feature or feature behavior change | `features/<feature-name>.md` | No |
...
```
## Changelog
[PROJ-123] Added Redis caching for user profiles
[PROJ-456] Migrated from H2 to PostgreSQL
```
