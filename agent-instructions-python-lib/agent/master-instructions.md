# Master Instructions — Python Library Agent
...
- Security-sensitive code (eval, exec, pickle, subprocess)

## Python Library Guardrails
- Semantic versioning (MAJOR.MINOR.PATCH)
- Backward compatibility in minor/patch releases
- Docstrings for all public functions/classes (Google or NumPy style)
- Type hints for all public API (PEP 484)
- Minimal runtime dependencies
- Clear deprecation warnings before removals (`warnings.warn` with `DeprecationWarning`)
...
- `infrastructure/environment.md` -> Python versions, platform support
- `infrastructure/dependencies.md` -> runtime packages, dev packages, external services
- `infrastructure/tooling.md` -> linters, test commands, CI/CD
- `infrastructure/publishing.md` -> package build, release process, PyPI
...
- `architecture/public-api.md` -> API surface, function signatures, type hints, semver rules
- `architecture/patterns.md` -> design patterns, conventions, quality checklist
- `architecture/error-handling.md` -> exception hierarchy, error classes, try/except patterns
- `architecture/data-flow.md` -> call lifecycle, hooks, transformations
