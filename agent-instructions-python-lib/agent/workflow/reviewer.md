...
- **Conventions:** `workflow/implementation.md` -- file locations, Python library conventions, docstrings, danger zones
- **Verification:** `workflow/testing.md` -- test and lint commands that must pass

### 4. Run verification
- Execute the phase's (or ticket's) verification commands (e.g. `pytest ...`, `mypy src/`, `ruff check src/`)
...
| Public functions/classes have docstrings | | Google or NumPy style |
| Public functions/classes have type hints | | PEP 484 compliant |
| Tests added/updated as required | | pytest coverage for new/changed behavior |
| Backward compatible (or version bumped) | | No breaking changes in minor/patch |
| Danger zones avoided or explicitly called out | | Public API, deps, pyproject.toml, publishing |
| Verification commands pass | | pytest, mypy, ruff |
...
- Public functions have docstrings: [Pass/Fail] -- [note]
- Public functions have type hints: [Pass/Fail] -- [note]
