...
## Python Library Phase Checks
- If touching public API: confirm docstrings + type hints + backward compatible
- If adding functions/classes: confirm pytest coverage for all public API
- If changing errors: confirm exception hierarchy consistency
- If modifying configuration: confirm defaults + validation
- If adding dependencies: confirm pyproject.toml updated + justified
