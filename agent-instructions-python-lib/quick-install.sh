#!/usr/bin/env bash
# Quick install script for agent-instructions-python-lib
# Usage: curl -fsSL https://raw.githubusercontent.com/rvk0106/coding-agent-instructions/main/agent-instructions-python-lib/quick-install.sh | bash
# Or with custom directory: curl ... | bash -s /path/to/python/project

set -euo pipefail

REPO_URL="https://github.com/rvk0106/coding-agent-instructions"
FRAMEWORK_DIR="agent-instructions-python-lib"
BRANCH="main"
TEMP_DIR=$(mktemp -d)
TARGET_DIR="${1:-.}"
...
echo "🚀 Installing agent-instructions-python-lib..."
...
# Check if it looks like a Python library project
if [[ ! -f "$TARGET_DIR/pyproject.toml" ]] && [[ ! -f "$TARGET_DIR/setup.py" ]] && [[ ! -f "$TARGET_DIR/setup.cfg" ]]; then
  echo "⚠️  Warning: This doesn't look like a Python library project."
  if [[ -t 0 ]]; then
...
echo "2. Plan a ticket: plan library for TICKET-ID"
