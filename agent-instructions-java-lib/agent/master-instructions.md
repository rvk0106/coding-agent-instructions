# Master Instructions — Java Library Agent
...
- pom.xml / build.gradle configuration changes
- Publishing to Maven Central
- Java version support changes
- Security-sensitive code (Runtime.exec, reflection on user input, deserialization)

## Java Library Guardrails
- Semantic versioning (MAJOR.MINOR.PATCH)
- Backward compatibility in minor/patch releases
- Javadoc on all public classes, interfaces, and methods
- Checkstyle/SpotBugs compliance
- Minimal runtime dependencies
- Clear @Deprecated annotations with @since tags before removals
- Null safety annotations (@Nullable/@NonNull) on public API
...
- `infrastructure/environment.md` → Java versions, JDK vendors, build tools
- `infrastructure/dependencies.md` → runtime deps, dev deps, external services
- `infrastructure/tooling.md` → build commands, linters, test commands, CI/CD
- `infrastructure/publishing.md` → Maven Central publishing, release process
- `infrastructure/security.md` → input validation, safe coding, dependency auditing

**Architecture** (technical design)
- `architecture/system-design.md` → module/package structure, components, data flows
- `architecture/public-api.md` → API surface, method signatures, Javadoc, semver rules
- `architecture/patterns.md` → design patterns, conventions, quality checklist
- `architecture/error-handling.md` → exception hierarchy, error classes
- `architecture/data-flow.md` → call lifecycle, thread safety, transformations
