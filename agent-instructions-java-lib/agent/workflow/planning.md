...
6. **Plan for reuse** → identify existing classes/interfaces to leverage
...
## Codebase Analysis (Java Library)
After loading instruction context, check these project files as relevant:
- `pom.xml` / `build.gradle` → project configuration, dependencies, version, plugins
- `src/main/java/` → source structure, package layout
- `src/main/java/.../api/` → public API surface
- `src/test/java/` → test patterns, existing coverage
- `src/main/resources/META-INF/services/` → SPI definitions (if used)
- `module-info.java` → module declaration (if JPMS used)
- `CHANGELOG.md` → versioning history
- `README.md` → documented public API and examples
- `config/checkstyle/` → style configuration (if present)

## Danger Zones (must flag in plan)
- Public API changes (breaking changes)
- Dependency additions/removals
- Java version support changes
- pom.xml/build.gradle metadata changes
