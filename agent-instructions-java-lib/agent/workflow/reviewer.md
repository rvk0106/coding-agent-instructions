...
- **Conventions:** `workflow/implementation.md` — file locations, Java library conventions, Javadoc, danger zones
- **Verification:** `workflow/testing.md` — test and lint commands that must pass

### 4. Run verification
- Execute the phase's (or ticket's) verification commands (e.g. `mvn test -Dtest=...`, `mvn checkstyle:check`)
...
| Public classes/methods have Javadoc | | @param, @return, @throws, @since |
| Null safety annotations present | | @Nullable/@NonNull on public API |
| Tests added/updated as required | | JUnit 5 tests for new/changed behavior |
| Backward compatible (or version bumped) | | No breaking changes in minor/patch |
| Thread safety documented | | If shared state or concurrent access |
| Danger zones avoided or explicitly called out | | Public API, deps, pom.xml, publishing |
| Verification commands pass | | JUnit, Checkstyle, SpotBugs, Javadoc |
...
- Public classes/methods have Javadoc: [Pass/Fail] — [note]
- Null safety annotations: [Pass/Fail] — [note]
- Tests added/updated: [Pass/Fail] — [note]
- Backward compatible: [Pass/Fail] — [note]
- Thread safety documented: [Pass/Fail] — [note]
