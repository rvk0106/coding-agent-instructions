...
```
LOAD:
  - architecture/system-design.md     → module/package structure, component layout
...
```
LOAD:
  - architecture/patterns.md          → builder/configuration pattern conventions
...
SKIP:
  ...
  - infrastructure/environment.md (unless Java version affected)
```
...
THEN load only the files that need updating based on what changed:
  - Package structure changed → architecture/system-design.md
