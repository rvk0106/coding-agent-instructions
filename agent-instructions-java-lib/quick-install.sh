#!/usr/bin/env bash
# Quick install script for agent-instructions-java-lib
# Usage: curl -fsSL https://raw.githubusercontent.com/rvk0106/coding-agent-instructions/main/agent-instructions-java-lib/quick-install.sh | bash
# Or with custom directory: curl ... | bash -s /path/to/java/project

set -euo pipefail

REPO_URL="https://github.com/rvk0106/coding-agent-instructions"
FRAMEWORK_DIR="agent-instructions-java-lib"
BRANCH="main"
TEMP_DIR=$(mktemp -d)
TARGET_DIR="${1:-.}"
...
echo "🚀 Installing agent-instructions-java-lib..."
...
# Check if it looks like a Java library project
if [[ ! -f "$TARGET_DIR/pom.xml" ]] && [[ ! -f "$TARGET_DIR/build.gradle" ]] && [[ ! -f "$TARGET_DIR/build.gradle.kts" ]]; then
  echo "⚠️  Warning: This doesn't look like a Java project."
  if [[ -t 0 ]]; then
