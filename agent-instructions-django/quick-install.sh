#!/usr/bin/env bash
# Quick install script for agent-instructions-django
# Usage: curl -fsSL https://raw.githubusercontent.com/rvk0106/coding-agent-instructions/main/agent-instructions-django/quick-install.sh | bash
# Or with custom directory: curl ... | bash -s /path/to/django/project

set -euo pipefail

REPO_URL="https://github.com/rvk0106/coding-agent-instructions"
FRAMEWORK_DIR="agent-instructions-django"
BRANCH="main"
TEMP_DIR=$(mktemp -d)
TARGET_DIR="${1:-.}"
...
echo "🚀 Installing agent-instructions-django..."
...
if [[ ! -f "$TARGET_DIR/manage.py" ]] && [[ ! -f "$TARGET_DIR/requirements.txt" ]] && [[ ! -f "$TARGET_DIR/pyproject.toml" ]]; then
  echo "⚠️  Warning: This doesn't look like a Django project."
  if [[ -t 0 ]]; then
