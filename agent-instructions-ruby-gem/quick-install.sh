#!/usr/bin/env bash
# Quick install script for agent-instructions-ruby-gem
# Usage: curl -fsSL https://raw.githubusercontent.com/rvk0106/coding-agent-instructions/main/agent-instructions-ruby-gem/quick-install.sh | bash
# Or with custom directory: curl ... | bash -s /path/to/gem/project

set -euo pipefail

REPO_URL="https://github.com/rvk0106/coding-agent-instructions"
FRAMEWORK_DIR="agent-instructions-ruby-gem"
BRANCH="main"
TEMP_DIR=$(mktemp -d)
TARGET_DIR="${1:-.}"
...
echo "🚀 Installing agent-instructions-ruby-gem..."
...
# Check if it looks like a Ruby gem project
if ! ls "$TARGET_DIR"/*.gemspec &>/dev/null && [[ ! -f "$TARGET_DIR/Gemfile" ]]; then
  echo "⚠️  Warning: This doesn't look like a Ruby gem project (no .gemspec or Gemfile)."
  if [[ -t 0 ]]; then
...
echo "2. Plan a ticket: plan gem for TICKET-ID"
